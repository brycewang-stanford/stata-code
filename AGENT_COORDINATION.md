...
| Core runner / schema changes | Claude | 2026-06-23 pass complete (roadmap Goals 1–4). (1) Error-taxonomy correctness: audited `RC_TO_KIND` vs StataCorp `[P] error` (Stata 19), fixed misclassified codes, added `RC_LABEL`/`label_for_rc()` for `error.rc_label`, expanded `suggestions_for()`. (2) Typed estimation contract: `results.estimation` coefficient table from `r(table)`/`e(b)`/`e(V)` (`core/estimation.py`). (3) Agent recovery contract: `error.recovery`/`recovery_for()`. (4) Reproducibility: `core/provenance.py` (`build_provenance`/`build_reproducible_do`). (5) Data-MCP handoff verifier: `core/handoff.py` (`verify_dataset`/`DatasetCheck`). New schema models Coefficient/EstimationResult/Recovery/Provenance (additive, v1.0); SCHEMA.md + JSON schema synced. Stretch work also landed: command-aware `estimation.command_family` + `diagnostics`, per-package provenance, `build_submission_package`, and `references/structured-results.md`. **Validated end-to-end against the live local Stata 18/MP**: added `tests/test_real_stata.py` (estimation/errors/provenance/handoff via pystata) and fixed a real bug it surfaced — the rc 199 "did you mean" never fired because the command-name regex did not match Stata's actual `"command <X> is unrecognized"` wording. Full suite 555 green (incl. 74+13 stata_required), ruff + mypy clean. Coordinated closely with Codex on exports/docs/JSON-schema/0.9.0 release | `stata_code/core/**`, `SCHEMA.md`, `schema/**`, `tests/test_real_stata.py` |
